...
- Work that is already in flight in user code (a mapping function, a retry attempt, a queue task, `f` in `timeout`) is **not interrupted** — JavaScript cannot interrupt it — but the operation does not wait for it: it rejects at once and the work's eventual result is dropped (a late rejection is swallowed, never left unhandled). Pass the same signal into that work when it should stop too. `consume` is the exception: it awaits callbacks already in flight before rejecting, as it does on failure.
