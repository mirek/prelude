...
const partial = <T extends Record<string, Predicate<unknown>>>(kvs: T): Predicate<{ [k in keyof T]?: Predicated<T[k]> }> => {
  const validators: Record<string, V.Validator<unknown>> = Object.create(null)
  for (const k in kvs) {
    validators[k] = toValidator(kvs[k])
  }
  return predicating(V.partial(validators) as V.Validator<{ [k in keyof T]?: Predicated<T[k]> }>)
}
