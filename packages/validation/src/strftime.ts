 ...
 */
/** Matches `value` against a strftime-style `format` (`%Y-%m-%d`, `%T`, ...); the failure records where matching stopped. */
export const strftime =
  (format: string): Validator<string> => {
    ...
    return value => {
      ...
      for (const token of tokens) {
        ...
        switch (token.type) {
          case 'regexp': {
            const m = rest.match(token.value)
            if (!m) {
              return fail(value, { kind: 'strftime', format, index: i })
            }
            i += m[0].length
            break
          }
          case 'literal':
            if (!rest.startsWith(token.value)) {
              return fail(value, { kind: 'strftime', format, index: i })
            }
